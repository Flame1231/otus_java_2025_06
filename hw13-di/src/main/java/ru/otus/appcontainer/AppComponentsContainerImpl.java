...
public class AppComponentsContainerImpl implements AppComponentsContainer {
    ...
    public AppComponentsContainerImpl(String classPath) throws Exception {
        ...
        Class<?>[] annotatedClasses = reflections
    ...
    private Object createComponent(Object config, Method method) {
        ...
        for (Class<?> classParam : List.of(method.getParameterTypes())) {
            if (!componentExists(classParam)) {
                throw new IllegalStateException(String.format(
                        "Not found component %s for create component %s",
                        classParam.getName(), method.getReturnType()));
            }
        ...
        try {
            ...
            throw new IllegalStateException(
                    String.format("Error create component %s", method.getReturnType()), e.getCause());
        }
        return component;
    }
    ...
    private boolean componentExists(Class<?> componentClass) {
        return appComponents.stream()
                .anyMatch(appComponent -> componentClass.isAssignableFrom(appComponent.getClass()));
    }
    ...
    public <C> C getAppComponent(Class<C> componentClass) {
        ...
        if (components.isEmpty()) {
            ...
            throw new IllegalArgumentException(
                    String.format("There are more than 1 component %s", componentClass.getName()));
        } else {
